...
def normalize_data(all_data, num):
...
def main():
    ...
    all_data, label = load_data(r'dataset/all_sample.mat')
    num = len(all_data) // 7
    all_data = normalize_data(all_data, num)
    
    dataset = myDataset(all_data, label)
    batch_size = 440  
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False)

    vecs = extract_features(Encoder, dataloader, device)
